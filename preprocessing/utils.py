...
def write_fasta(output_fasta_path: str,
    ...
    assert path.isdir(path.abspath(path.dirname(output_fasta_path))), \
        f'Output Directory \
            "{os.path.abspath(path.dirname(output_fasta_path))}" \
            Does not exists'
...
def get_blosum62_substitution_matrix() -> align.SubstitutionMatrix:
    """
    ...
    for i in range(1, len(seqs_items_list)):
        for j in range(i):
            ...
            if key1 in seqs_dict and key2 in seqs_dict:
                if are_seqs_homologs(seq1, seq2, substitution_matrix,
                                     threshold=threshold):
